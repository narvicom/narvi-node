import { getDataFromArgs, getNarviRequestHeaders, getOptionsFromArgs, getNarviRequestSignaturePayload, makeURLInterpolator, protoExtend, getNarviRequestSignature, stringifyRequestData, } from '../../utils/utils';
...
NarviResource.method = narviMethod;
NarviResource.MAX_BUFFERED_REQUEST_METRICS = 100;
/**
 * Encapsulates request logic for a Narvi Resource
 */
function NarviResource(narvi, deprecatedUrlData) {
    this._narvi = narvi;
    if (deprecatedUrlData) {
        throw new Error('Support for curried url params was dropped in narvi-node v7.0.0. Instead, pass two ids.');
    }
...
NarviResource.prototype = {
    ...
    _getRequestOpts(requestArgs, spec, overrideData) {
        ...
        const urlData = urlParams.reduce((urlData, param) => {
            ...
            if (typeof arg !== 'string') {
                throw new Error(`Narvi: Argument "${param}" must be a string, but got: ${arg} (on API request to \`${requestMethod} ${path}\`)`);
            }
            urlData[param] = arg;
            return urlData;
        }, {});
        ...
        if (args.filter((x) => x !== null).length) {
            throw new Error(`Narvi: Unknown arguments (${args}). Did you mean to pass an options object? See https://github.com/narvi/narvi-node/wiki/Passing-Options. (on API request to ${requestMethod} \`${path}\`)`);
        }
    ...
    _makeRequest(requestArgs, spec, overrideData) {
        return new Promise((resolve, reject) => {
            ...
            const methodHasPayload = opts.requestMethod === 'POST' ||
            ...
            const addNarviHeaders = (headers) => (Object.assign(Object.assign({}, headers), getNarviRequestHeaders({
