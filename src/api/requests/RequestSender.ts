...
import { HttpClient, HttpClientResponseInterface } from '../../http/HttpClient'
import {
  RequestHeaders,
  RequestEvent,
  ResponseEvent,
...
export class RequestSender {
  protected _narvi: NarviObject

  constructor(narvi: NarviObject) {
    this._narvi = narvi
  }
  ...
  _makeResponseEvent(
    ...
    return removeNullish({
      api_version: headers['narvi-version'] as string,
      account: headers['narvi-account'] as string,
  ...
  static _generateConnectionErrorMessage(requestRetries: number): string {
    return `An error occurred with our connection to Narvi.${
  ...
  _request(
    ...
    const makeRequest = (
      ...
      const req = this._narvi
        ...
        .makeRequest(
          host || this._narvi.getApiField('host'),
          this._narvi.getApiField('port'),
          path,
          method,
          headers,
          requestData,
    ...
    const prepareAndMakeRequest = (error: Error | null, data: string): void => {
      ...
      this._narvi.getClientUserAgent((clientUserAgent: string) => {
    ...
    if (requestDataProcessor) {
      ...
      prepareAndMakeRequest(
        null,
        stringifyRequestData(data || {}, methodHasPayload),
      )
    }
  }
}
